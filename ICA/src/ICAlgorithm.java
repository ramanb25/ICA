...
public class ICAlgorithm
{
	...
	// ICA parameters
	int numOfCountries = 9;               		// Number of initial countries
	int numOfInitialImperialists = 4;      		// Number of initial imperialists
	...
	int receivingTrucksQty;						// Problem dimension
	 ...
	 */
	protected double[] runICA(int maxEvals)
	{
		...
		double[] minimumCostRedux = Arrays.copyOf(minimumCost, lastDecade+1);
		...
		//bestSolution=initialCountries[2];
		return bestSolution;
	}
	...
	private double[][] generateNewCountries(int numberOfCountries, int dimension, double[] minVector, double[] maxVector, Random rand) 
	{
		...
		{
			ArrayList<Integer> r = new ArrayList<Integer>(5);
		   ...
		   Collections.shuffle(s);
			...
			{
				if(j<5){
					countriesArray[i][j] = r.get(j);
				}
				else
				{
					countriesArray[i][j] = s.get(j-5);
				}
			}
		}
	...
	private double[] getCountriesCosts(double[][] countriesArray) 
	{
		...
		{
			//utils.printArray("Sequence::", countriesArray[i]);
			FitnessFunction objnew=new FitnessFunction();
			costsVector[i] = objnew.getFitnessValue(countriesArray[i]);
			/*fitnessFunc.ans=0;
			fitnessFunc.idr=0;
			fitnessFunc.ids=0;
			fitnessFunc.storeType1=0;
			fitnessFunc.storeType2=0;*/
		}
		return costsVector;
	}
	...
	private void assimilateColonies(Empire theEmpire)
	{
		...
		{
			for(int j=0; j<receivingTrucksQty; j++)
			{
				array[i][j] = repmatArray[i][j] - theEmpire.getColoniesPosition()[i][j];
			}
			for(int j=receivingTrucksQty; j<problemDimension; j++)
			{
				array[i][j] = repmatArray[i][j] - theEmpire.getColoniesPosition()[i][j];
			}
			//utils.printArray("array::",array[i]);
		}
		...
		for(int i=0; i<array.length; i++)
		{
		   ...
		   Collections.shuffle(s);
			...
			{
				if(j<5){
					coloniesPosition[i][j] = r.get(j);
				}
				else
				{
					coloniesPosition[i][j] = s.get(j-5);
				}
			}
			/*for(int j=0; j<array[0].length; j++)
			{
				coloniesPosition[i][j] = theEmpire.getColoniesPosition()[i][j] + 2 * assimilationCoefficient  * array[i][j];

			}*/

			//utils.printArray("Countries::", coloniesPosition[i]);
		}
	...
	public void reset()
	{
		...
		r = new Random(seed);
		empiresList = new Empire[numOfInitialImperialists];	// List of Empires
		minimumCost = new double[numOfDecades];	// The minimum cost of each decade
		meanCost = new double[numOfDecades];		// The mean cost of each decade
